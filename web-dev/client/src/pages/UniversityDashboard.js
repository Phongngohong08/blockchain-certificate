  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // First try to get dashboard-specific data
        const response = await fetch(`${API_URL}/api/university/dashboard`, {
          method: 'GET',
          headers: {
        ...
        if (response.ok) {
          ...
          // Fallback: get certificates directly
          const certsResponse = await fetch(`${API_URL}/api/university/certificates`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
          ...
          if (certsResponse.ok) {
            ...
            const certificates = certsData.certificates || certsData || [];
            
            setStats({
              totalCertificates: certificates.length,
              certificatesThisMonth: certificates.filter(c => {
                const date = new Date(c.dateOfIssue);
                const now = new Date();
