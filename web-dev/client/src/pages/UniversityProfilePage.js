...
const UniversityProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUniversityProfile = async () => {
      try {
        ...
        // Use the university data directly from data or from data.university
        const university = data.university || data;
        
        setForm({
          name: university.name || '',
          email: university.email || '',
          universityId: university.universityId || '',
          address: university.location || university.address || '',
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/university/change-password`, {
        method: 'POST',        headers: {
          'Content-Type': 'application/json',
