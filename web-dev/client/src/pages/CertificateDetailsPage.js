...
const CertificateDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchCertificate = async () => {
      try {
        // First try the specific endpoint based on user type
        let endpoint = userType === 'university' 
        ...
        // If not found, try the general certificate endpoint
        if (!response.ok) {
          response = await fetch(`${API_URL}/api/certificate/${id}`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
          ...
          if (!response.ok) {
            throw new Error(data.error || data.message || 'Failed to fetch certificate');
          }
        }
  ...
  const handleShareCertificate = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/student/certificates/${id}/share`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleRevokeCertificate = async () => {
    if (window.confirm('Are you sure you want to revoke this certificate? This action cannot be undone.')) {
      try {
        setLoading(true);
        const response = await fetch(`${API_URL}/api/university/certificates/${id}/revoke`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
