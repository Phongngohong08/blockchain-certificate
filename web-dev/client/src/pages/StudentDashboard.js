...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchStudentCertificates = async () => {
      try {
        const response = await fetch(`${API_URL}${API_CONFIG.ENDPOINTS.STUDENT.CERTIFICATES}`, {
          method: 'GET',
          headers: {
        ...
        if (response.ok) {
          ...
          console.error('Error fetching certificates:', data.error || data.message);
        }
      } catch (error) {
            ...
            <Card.Body>
              ...
              {loading ? (
                ...
                <Table hover responsive>
                  ...
                  <tbody>                    {certificates.slice(0, 5).map((cert) => (
                      <tr key={cert._id}>
